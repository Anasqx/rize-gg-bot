...
export async function memberAdmin(i,rewards,reply){
 ...
 if(action==='tagsettings')return reply('رتبة تاق السيرفر',`الرتبة: ${rewards.store.get('tag:role')?`<@&${rewards.store.get('tag:role')}>`:'غير محددة'}\nأنشئ رتبة للتاق فقط ثم اخترها. نتحقق من تاق هذا السيرفر نفسه.\nالفحص عند النشاط وفتح الرصيد، مع مراجعة دورية للأعضاء المسجلين. إزالة التاق تسحب فقط الرتبة التي منحها البوت.`,[row(new RoleSelectMenuBuilder().setCustomId('reward:tagrole').setPlaceholder('اختر رتبة التاق')),row(button('reward:tagcheck','فحص عضو')),back()]);
 if(action==='tagrole'){
