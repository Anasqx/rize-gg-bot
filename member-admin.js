...
export async function memberAdmin(i,rewards,reply){
 ...
 if(action==='tagsettings')return reply('رتبة تاق السيرفر',`الرتبة: ${rewards.store.get('tag:role')?`<@&${rewards.store.get('tag:role')}>`:'غير محددة'}\nأنشئ رتبة للتاق فقط ثم اخترها. نتحقق من تاق هذا السيرفر نفسه.\nالفحص عند تحديث التاق، مع مراجعة دورية. إزالة التاق تسحب رتبة التاق حتى لو مُنحت يدويًا. الرتبة فقط؛ دبل الخبرة تديره الإدارة من بوت آخر.`,[row(new RoleSelectMenuBuilder().setCustomId('reward:tagrole').setPlaceholder('اختر رتبة التاق')),row(button('reward:tagcheck','فحص عضو')),back()]);
 if(action==='tagrole'){
